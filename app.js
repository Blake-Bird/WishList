/* ============================================================
   ...
   Single-file animation + sound engine (insane, cinematic, smooth)
   Tech: GSAP 3.12 + ScrollTrigger, Lenis, tiny Three.js accents, Canvas FX, WebAudio
   ============================================================ */
...
function pin(tl, el, start="top top", dur="+=200%") {
  ScrollTrigger.create({ animation: tl, trigger: el, start, end: dur, scrub: true, pin: true, anticipatePin: 1 });
}
...
const AudioEngine = (() => {
  ...
  function whoosh(){ // filtered noise whoosh for transitions
    ensure();
    const now = ctx.currentTime;
...
/* -----------------------------
   PREFETCH IMAGES (matches assets list from the plan)
--------------------------------*/
preload([
...
/* ==================================================================
   SCENES (exact order)
   Each scene uses a distinct technique: no repeat grammar.
   ================================================================== */

/* 1) S63 AMG — tri-frame + chrome glint + wheel FLIP */
(function s63Scene(){
  const el = $('.s-s63'); if(!el) return;
  const frames = $$('.s63', el);
  ...
  // spec chips pop-in stagger
  tl.from(chips, {y:20, opacity:0, stagger:0.07, duration:0.3}, 0.1);

  // chrome "glint" sweep (gold line traveling across image)
  ...
  // wheel FLIP: quick rim close-up spin then resolve to hero (simulate by scaling a masked circle)
...
/* 2) Oliver Cabell Drivers — suede fiber macro + ghost footsteps trail */
(function driversScene(){
  const el = $('.s-drivers'); if(!el) return;
  const img = $('img', el);
  ...
  // ghost footsteps trail (mouse move)
...
/* 3) LF Cashmere Hoodie — cloth drape + stitch shimmer */
(function hoodieScene(){
  const el = $('.s-hoodie'); if(!el) return;
  const img = $('img', el);
  const tl = gsap.timeline();
  if(!isReduced){
    ...
    // stitch shimmer (moving gradient overlay)
...
/* 4) Bulova Sutton — tick-sync, caustic arc, shutter date */
(function watchScene(){
  const el = $('.s-watch'); if(!el) return;
...
/* 6) Clubmaster — lens refraction + sun-kiss sweep; mouse tilt */
(function clubmasterScene(){
  const el = $('.s-clubmaster'); if(!el) return;
...
/* 7) Herod — vapor particle field (canvas) */
(function herodScene(){
  const el = $('.s-herod'); if(!el) return;
  ...
  function step(){
    ...
    for(const p of P){
      ...
      if(p.y < -10 || p.a < 0.02){ // respawn bottom
...
/* 8) Cream Linen Blazer — pattern sketch + stitch draw */
(function creamBlazerScene(){
  const el = $('.s-cream-blazer'); if(!el) return;
...
/* 10) Cashmere Joggers — run-in motion blur then relax slump */
...
/* 19) Velvet Brown — “pet the pile” sheen reaction */
...
/* 23) Suits Blueprint — plotter draw + fold */
(function suitsScene(){
...
/* 27) Board Games — parallax shelf + card fan open */
(function gamesScene(){
...
/* 28) Ultima RS — blueprint -> photoreal morph + year rail */
(function ultimaScene(){
  const el = $('.s-ultima'); if(!el) return;
  ...
  function render(){
    ...
    ctx.stroke();
    // carbon weave hints (diagonal hatch)
...
/* 37) Fragrance constellation — canvas particle system */
(function fragsScene(){
  const el = $('.s-frags'); if(!el) return;
...
window.addEventListener('load', ()=>{
  // micro gradient pulse on hero line so first impression breathes luxury
